...
- 📄 **`index.html`**：
  - html lang 属性改为 `ja`
  - 标题改为 "YUIChat - AI ナレッジベースアシスタント"
  - 描述改为 "企業向け AI ナレッジベース管理プラットフォーム。ドキュメントアップロード、スマート Q&A、知識検索をサポート"
  - og:locale 默认值改为 `ja_JP`
...
- **整体样式调整**：
  - 📄 **`src/pages/AllProjectsPage.tsx`**：
    - 项目卡片背景从蓝色渐变改为 YUIChat 淡紫色主题（from-primary/80 to-primary）
    - 统一使用紫色主题的几何装饰图案
    - 在项目名称下方居中显示项目说明
    - 项目说明仅在有内容时显示，限制一行，超出部分用...代替
    - 使用 `line-clamp-1` 实现文本截断效果

## 1.3.16 (2026-01-25)

### 欢迎语和推荐问题自动翻译填充功能

- **功能描述**：在项目设置中，用户只需输入一种语言的欢迎语或推荐问题，即可一键自动翻译并填充其他语言

- **后端新增翻译 API**：
  - 📄 **`backend_py/app.py`**：
    - 新增 `/api/translate` 端点：使用 GPT-4o-mini 进行多语言翻译
    - 支持中文、英文、日文三种语言互译
    - 翻译结果保持原文的语气和风格

- **前端设置页面优化**：
  - 📄 **`src/pages/SettingsPage.tsx`**：
    - 欢迎语输入区域：每个语言输入框旁添加"自动填充其他语言"按钮
    - 推荐问题输入区域：每个语言分组旁添加"自动填充其他语言"按钮
    - 添加提示文本说明自动填充功能的使用方式
    - 翻译过程中显示 loading 状态

- **多语言文本**：
  - 📄 **`src/i18n.ts`**：
    - 新增 `autoFillOtherLangs`：自动填充其他语言按钮文本
    - 新增 `autoFillHint`：欢迎语自动填充提示
    - 新增 `autoFillQuestionsHint`：推荐问题自动填充提示
    - 新增 `autoFillSuccess`：自动填充成功提示
    - 新增 `autoFillFailed`：自动填充失败提示
    - 新增 `noSourceTextToTranslate`：无内容可翻译提示

## 1.3.15 (2026-01-25)

### 新增 AI 回复性能瓶颈分析文档

- 📄 **`docs/AI_RESPONSE_PERFORMANCE_ANALYSIS.md`**（新建）：
  - 全面分析当前架构的性能瓶颈
  - 主要瓶颈识别：LLM API 调用 (GPT-4o) 占总延迟 60-70%
  - 开源方案对比：LLM 模型、RAG 框架、向量数据库、Embedding 模型
  - 服务器架构问题分析：跨地域调用、单 worker、冷启动等
  - 优化建议及优先级排序
  - 预期优化效果：响应时间从 3-7秒 降至 1-3秒
...
- **商用场景优化**：
  - 默认缓存有效期从 24 小时延长至 **7 天（168小时）**
  - 新增**热门问题自动延长 TTL** 机制：
    - 命中次数 ≥ 5 次的问题自动延长 TTL 至 4 倍（28天）
    - 配置项：`QA_CACHE_HOT_THRESHOLD`（默认 5）
    - 配置项：`QA_CACHE_HOT_TTL_MULTIPLIER`（默认 4）
  - 优先用户体验，降低 LLM 调用成本
...
- **数据库扩展**：
  - `conversations` 表新增 `source_type` 字段：区分测试对话('test')和公开用户对话('public')
  - `conversations` 表新增 `session_id` 字段：用于标识公开用户（通过 localStorage 生成的唯一ID）
  - `conversations` 表新增 `expires_at` 字段：支持数据保存周期管理
  - 新增 `conversation_settings` 表：存储每个知识库的对话保存周期配置
  - 新增 `get_frequent_questions()` 数据库函数：高频问题统计
  - 新增 `get_conversation_stats()` 数据库函数：对话统计数据
  - 更新 RLS 策略：支持公开模式下的对话记录保存

- **前端新增页面和组件**：
  - 📄 **`src/pages/ConversationDataPage.tsx`**：对话数据管理页面
    - 日期范围选择筛选
    - 来源类型筛选（测试/公开/全部）
    - 关键词搜索功能
    - 对话记录列表展示
    - 展开查看完整对话内容
    - 数据保存周期设置（永久、3个月、6个月、1年）
  
- **扩展数据看板**：
  - 📄 **`src/pages/DashboardPage.tsx`**：
    - 新增对话统计卡片：总对话数、今日对话、总消息数
    - 区分显示测试对话和公开用户对话数量
    - 新增高频问题 Top 10 排行榜
...
- **扩展对话服务**：
  - 📄 **`src/services/conversationService.ts`**：
    ...
    - `getConversationCount()`：获取对话总数（用于分页）
...
- **公开模式对话保存**：
  - 📄 **`src/components/ChatInterface.tsx`**：
    - 公开模式现在也会创建对话记录（source_type='public'）
    - 公开模式下的消息也会保存到数据库
    - 使用 session_id 标识公开用户

- **路由和导航更新**：
  - 📄 **`src/App.tsx`**：新增 `/conversation-data` 路由
  - 📄 **`src/components/Sidebar.tsx`**：项目菜单新增"对话数据"入口

- **多语言支持**：
  - 📄 **`src/i18n.ts`**：新增对话统计相关的中日英三语翻译
...
### 修复 GCP Cloud Run 部署脚本优化问题

- 🐛 **问题分析**（ChatGPT 审核反馈）：
  1. **双 revision 问题**：deploy 后再 update env vars 导致每次部署创建 2 个 revision
  2. **SHORT_SHA 变量问题**：Cloud Build 自定义变量必须以 `_` 开头
  3. **Kaniko 缓存未生效**：缺少 `--cache-repo` 参数导致层缓存无法复用

### 核心修复

1. **避免双 revision**：
   - 非首次部署时，在同一次 `deploy` 命令中设置 `CLOUD_RUN_SERVICE_URL`
   - 仅首次部署时才需要补一次 `update`（一次性成本）
   - 日常部署从 2 个 revision 减少到 1 个

2. **修复 substitution 变量**：
   - `SHORT_SHA` 改为 `_SHORT_SHA`
   - Cloud Build 自定义变量必须以下划线开头

3. **启用 Kaniko 层缓存**：
   - 添加 `--cache-repo=${_IMAGE}/cache` 参数
   - 缓存保留 7 天，显著加速后续构建
...
- 📄 **`backend_py/deploy-gcp.sh`**：
  - 版本更新到 v1.3.10
  - 修复双 revision 问题
  - 使用 `_SHORT_SHA` 替代 `SHORT_SHA`
  - 优化部署流程提示

- 📄 **`backend_py/cloudbuild.yaml`**：
  - 添加 `--cache-repo` 启用真正的层缓存
  - 使用 `_SHORT_SHA` 替代 `SHORT_SHA`
  - 添加 `_SHORT_SHA` 默认值

### 预期效果

- 日常部署只创建 1 个 revision（之前是 2 个）
- Kaniko 缓存命中后，构建时间预计从 400s 降至 60-120s
- 部署日志/回滚更清晰

### 追加修复 - 添加 .gcloudignore

- 📄 **`backend_py/.gcloudignore`**：
  - 发现每次部署上传 56231 个文件，1.3GB（包含 venv、chroma_db 等）
  - 添加 .gcloudignore 排除不必要的文件
  - 预计上传体积从 1.3GB 降至 <50MB
  - 这是构建慢的**主要原因之一**
...
### 优化RAG检索准确性 - 解决文档开头关键信息检索不到的问题

- 🐛 **问题**：
  - 用户反馈：询问"新法人税法の施行日"时，系统回答"上下文中没有提供具体的施行日期"
  - 但原网页第一行就有"令和3年3月1日 施行"的日期信息
  - 原因分析：日期信息在网页顶部，被分割到单独的chunk中，与主体内容分离
  - 语义检索时，日期chunk与"法人税法の施行日"问题的相似度不够高

### 核心改进

1. **文档切分优化**：
   - 为每个chunk添加【文档标题】和【来源】前缀
   - 确保每个块都包含足够的上下文信息
   - chunk_overlap 从200增加到300，减少跨块信息丢失

2. **检索数量增加**：
   - RETRIEVE_K 默认值从8增加到12
   - 确保能检索到更多相关文档，包括可能排名靠后的关键信息

### 更新内容

- 📄 **`backend_py/workflow.py`**：
  - 新增文档内容enrichment逻辑：为每个文档添加标题和来源前缀
  - chunk_overlap 从200调整为300
  - RETRIEVE_K 默认值从8调整为12
  - 优化系统提示词：避免AI以"根据提供的上下文"等词语开头，改为直接回答

- 📄 **`src/components/ChatInterface.tsx`**：
  - 新增语言切换监听，切换语言时自动刷新对话框
  - 显示对应语言的欢迎语和推荐问题
